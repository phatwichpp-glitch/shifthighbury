...
import { RECEIPT_SERVICE_LABEL } from '../../lib/appConfig';

export function ReceiptDocument({ id, receipt, accentColor, logoUrl, instituteName, footerNote, signatureUrl }) {
  ...
  return (
    <div id={id} style={{ background: 'white', margin: '0 auto', width: '72mm',
      ...
      <div style={{ textAlign: 'center', marginBottom: '10px' }}>
        ...
        <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.12em', textTransform: 'uppercase' }}>ใบเสร็จรับเงิน</div>
      </div>
      ...
      <R label="อ้างอิงบิล"  value={receipt.invoice[INVOICE.NUMBER]} />
      <R label="ลูกค้า"     value={receipt.studentName} bold />
      <div style={{ borderTop: '1px dashed #aaa', margin: '8px 0' }} />
      <div style={{ fontSize: '12px', fontWeight: 600, marginBottom: '2px' }}>{RECEIPT_SERVICE_LABEL}</div>
      <div style={{ fontSize: '11px', color: '#555', marginBottom: '6px' }}>
        อ้างอิง {receipt.invoice[INVOICE.NUMBER]} · {receipt.invoice[INVOICE.TOTAL_HOURS]} ชั่วโมง
      </div>
      <div style={{ borderTop: '1px dashed #aaa', margin: '8px 0' }} />
